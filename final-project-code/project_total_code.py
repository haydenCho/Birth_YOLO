...
bluetooth_serial_port_led = 'COM10'  # 윈도우에서는 'COMX' 형식으로 설정
bluetooth_serial_port_room = 'COM8'  # 윈도우에서는 'COMX' 형식으로 설정
...
def calculate_height(object_height_in_frame, camera_height_mm, image_height_px):
    actual_object_height_mm = (object_height_in_frame * camera_height_mm) / (image_height_px * math.cos(math.radians(camera_angle_deg))) / 1.2 ### 키 수정한 부분
...
last_pose_detection_time = 0
pose_detection_duration = 15 # 아기 인식 및 자세 감지 시 유지되는 시간

# 미디어 파이프로 포즈 추정
# 미디어 파이프로 포즈 추정
with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
    while cap.isOpened():
        # 30초 동안 객체 감지를 중단하는 조건 추가
        if time.time() - last_pose_detection_time < pose_detection_duration:
            remaining_time = pose_detection_duration - (time.time() - last_pose_detection_time)
            print(f"Skipping object detection. Time remaining: {remaining_time:.2f} seconds")
            time.sleep(1)  # 1초씩 대기하면서 시간을 확인
            continue  # 감지를 중단하고 루프를 다시 시작
        ...
        for obj in results.xyxy[0]:
            ...
            if distance < 300:      # 2m 이상인 경우 측정 x
                # Draw bounding box
                ...
                if time.time() - last_pose_detection_time < pose_detection_duration:
                    remaining_time = pose_detection_duration - (time.time() - last_pose_detection_time)
                    print(f"Skipping pose check. Time remaining: {remaining_time:.2f} seconds")
                    continue
                ...
                if distance < 200:
                    if class_label == 'baby' or class_label == 'stroller':
                        ...
                        open()
                        last_pose_detection_time = time.time()
